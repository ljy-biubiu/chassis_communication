...
int query_serial_data(int fd)
{
    ...
    if(ret > 0)
    {
        ...
        for(i=0; i<ret; i++)
        {
            DEBUG_PRINTF2("%d.", serial_recv.buf[pos + i]);
            //DEBUG_PRINTF2("%c", serial_recv.buf[pos + i]);
        #if 0
            if(i == ret - 1)
            {
                DEBUG_PRINTF2_L2("/");
...
/*****************************************************************************
 Prototype    : receive_serial_frame
 Description  : receive serial data and identify frames to frame queue.
 Input        : void  
 Output       : None
 Return Value : 
 Calls        : 
...
*****************************************************************************/
int receive_serial_frames(void)
{  
    int fd;
...
{  
    if(cmd >= CMD_MAX)
    {
        DEBUG_PRINTF("Invalid cmd type[%d] when init serial frame!!!\n", cmd);
        return -1;
    }
...
{
   if(snd_frame_queue.cnt >= (MAX_SEND_FRAME_QUEUE/4)*3 )
    {
    ...
    //if(snd_frame_queue.cnt > 0)
    {
        ...
        serial_frame_type *frame_with_cmd = (serial_frame_type*)frame->buf;
        switch(snd_frame_queue.state)
        {
            case NORMAL_SEND: //send a frame;
                ...
                if((snd_frame_queue.resend[snd_frame_queue.head] & 0xff) != 0)
                {
                    gettimeofday(&tv, NULL);
                    if(frame_with_cmd->cmd == 203){
                        ts_set = tv.tv_sec*10 + tv.tv_usec/100000 + 10; //unit per 100 ms, set 1s later
                        DEBUG_PRINTF("Acorrding param, set resend[%d] timer, set delay time: %ds\n", (snd_frame_queue.resend[snd_frame_queue.head] & 0xff),1);
                    }else{
                        ts_set = tv.tv_sec*10 + tv.tv_usec/100000 + 30;  //unit per 100 ms, set 2s later
                        DEBUG_PRINTF("Acorrding param, set resend[%d] timer, set delay time: %ds\n", (snd_frame_queue.resend[snd_frame_queue.head] & 0xff),3);
                    }
            ...
            case TRY_RESEND:
                ...
                if( (snd_frame_queue.resend[snd_frame_queue.head] & 0xff) != 0)
                {
                    ...
                    if(ts_cur >= ts_set)  //timesup
                    {
                        ...
                        snd_frame_queue.resend[snd_frame_queue.head]--;

                        if(0 == (snd_frame_queue.resend[snd_frame_queue.head] & 0xff))
                        {
                            snd_frame_queue.state = END_SEND;  
                        }
						else
